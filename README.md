...
**Java points-to analysis (template grammar, ~858 indices):** CFRA is **1.5-2x slower** than FastMatrix. The template grammar expands to hundreds of indexed rules, resulting in hundreds of individual MxM calls per iteration on very sparse matrices. Each cuBool kernel launch incurs a fixed overhead that dominates when the actual computation takes microseconds. The planned **block matrix optimization** ([Muravev](https://github.com/FormalLanguageConstrainedPathQuerying/CFPQ_PyAlgo), Section 3.7) addresses this by replacing hundreds of small MxM calls with a single large block-matrix multiplication.
