...
"""
Usage:
    python3 plot_results.py results.csv [--output plots/]
"""
...
def plot_speedup(stats: pd.DataFrame, output_dir: str, baseline: str = None):
    ...
    if baseline not in solvers:
        print(f"Baseline solver '{baseline}' not found in data, skipping speedup")
        return
    ...
    if not other_solvers:
        print("Only one solver, cannot compute speedup")
        return
    ...
    ax.set_xlabel("Graph")
    ax.set_ylabel("Speedup (baseline / solver)")
...
def main():
    parser = argparse.ArgumentParser(description="CFPQ solver comparison plots")
    ...
    parser.add_argument("--baseline", "-b", default=None,
                        help="Baseline solver name for speedup (default: first)")
    ...
    print(f"  {len(df)} successful runs, {df['solver'].nunique()} solvers, "
          f"{df['graph'].nunique()} graphs")
